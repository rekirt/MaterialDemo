...
import com.example.lc.materialdemo.utils.Logger;
...
public class ScrollZoomLayoutManager extends RecyclerView.LayoutManager {
    ...
    //the space between each items
    private int itemSpace = 0;
    private int offsetDistance;

    //max scale rate defalut is 1.2f
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        // count===0
        // count===2
        Logger.e(getItemCount()+"==count==="+getChildCount());
...
//                    scrap.setScaleY(scale);
                    ...
                    View group = recycler.getViewForPosition(i);
                    measureChildWithMargins(group, 0, 0);
                    View scrap = group.findViewById(R.id.ll_content);
                    addView(group);
                    int x = itemsX.get(i) - offsetScroll;
                    float scale = calculateScale(startLeft+x);
                    scrap.setRotation(0);
                    layoutDecorated(group,startLeft+x, startTop,startLeft+x + mDecoratedChildWidth, startTop + mDecoratedChildHeight);
                    itemAttached.put(i, true);
                    scrap.setScaleX(scale);
                    scrap.setScaleY(scale);
                }
...
//            v.setScaleY(scale);
            ...
            View v = getChildAt(i);
            View scrap = v.findViewById(R.id.ll_content);
            float scale = calculateScale(v.getLeft());
            layoutDecorated(v,v.getLeft()-willScroll,v.getTop(),v.getRight()-willScroll,v.getBottom());
            scrap.setScaleX(scale);
            scrap.setScaleY(scale);
